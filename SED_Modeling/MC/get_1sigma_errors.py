...
def get_1sig(x,x_best):
    ...
    if x_best>0 and len(x_low)>0:
        x_low.sort()
        xmin = x_best - x_low[np.int32((len(x_low)-1)*(1.-0.683))]
    else: 
        xmin = 0
    if len(x_hig)>0:
        x_hig.sort()
        xmax = x_hig[np.int32((len(x_hig)-1)*0.683)] - x_best
    else:
        xmax = 0

    #Return the 68.3% ranges. 
    return xmin,xmax
...
cato = open("1_sigma_errors.dat","w")
f22  = open("../double.22")
for line in rcat.readlines():
    ...
    ebv_l_err_do , ebv_l_err_up  = get_1sig(ebv_l ,ebv_l_best )
    ebv_h_err_do , ebv_h_err_up  = get_1sig(ebv_h ,ebv_h_best )
    L6um_l_err_do, L6um_l_err_up = get_1sig(L6um_l,L6um_l_best)
    L6um_h_err_do, L6um_h_err_up = get_1sig(L6um_h,L6um_h_best)
