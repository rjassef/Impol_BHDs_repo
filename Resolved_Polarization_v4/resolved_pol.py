...
class ResolvedPol(object):
    ...
    def get_pol(self, regularize_PSF=True, special_ob_pairs=None, align_images=True, no_processing=False, common_seeing_all_obs=True, show_diagnostics=True, blurr_PSF_FWHM=None):
        ...
        for iob, ob_comb in enumerate(ob_combs):
            ...
            fits.writeto("work/imerr.fits", h[1].data[40:900,300:1700], overwrite=True)
            h.close()
            ...
            for fname in fnames:
                ...
                if fname!=worse_psf_fname:
                    ...
                    fits.writeto("work/terr.fits", h[1].data[40:900,300:1700], overwrite=True)
                    ...
                    #We do reverse matching (option -R), where the image with the worst seeing, im.fits, is taken as the template, and the science image, t.fits, is blurred to match the template. This is optimal for what we want to do, but is generally uncommon since typically the template has better seeing and is blurred to match the science image.
                    #command = "ImageMatch -v -R -preserve -cfg sample.cfg -m work/im.fits -snx {} -sny {} -sig err work/t.fits".format(self.bhd_coords[0]-300, self.bhd_coords[1]-40)
                    command = "ImageMatch -v -R -preserve -cfg sample.cfg -m work/im.fits -sig err work/t.fits"
                    print(command)
                    ...
                    hh = fits.open("work/tSN.fits")
                    h[0].data[40:900,300:1700] = hh[0].data

                    #Also get the error image. The code outputs the error image of the subtraction. So to backtrack the blurred error of the science image, we need to subtract in quadrature the template image error.
                    hherr = fits.open("work/tdiff_sigma.fits")
                    imerr = fits.open("work/imerr.fits")
                    blurred_err_im = (hherr[0].data**2-imerr[0].data**2)**0.5
                    h[1].data[40:900,300:1700] = blurred_err_im
    ...
    def plot_imagematch(self, fname):
        ...
        blurred  = fits.getdata("work/tSN.fits")
